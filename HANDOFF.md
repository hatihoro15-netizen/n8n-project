...
## A) 상태 요약
- **현재 프로젝트**: 온카스터디 (n8n-web — YouTube 영상 자동 제작 관리 플랫폼)
- **Current Status**: Phase 2 완료, Phase 3 부분 진행
...
- **Required Secrets**: DATABASE_URL, JWT_SECRET, N8N_API_KEY, N8N_BASE_URL, N8N_WEBHOOK_BASE, CORS_ORIGIN, ADMIN_USERNAME, ADMIN_PASSWORD, PRODUCTION_TIMEOUT_MINUTES (모두 VPS `.env`에서 관리, 코드 하드코딩 금지)
...
## G) 다음 세션 시작용 메시지 (복붙)
> n8n-web 프로젝트 (YouTube 영상 자동 제작 관리 플랫폼). Phase 2(제작 컨트롤) 완료, Phase 3 API 완성 상태. 다음 작업: 프롬프트 에디터 UI(Monaco) + 캐릭터 관리 UI 구현. 브랜치: feature/web-app.
